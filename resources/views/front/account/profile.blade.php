            ...
            <div class="col-lg-9">
                ...
                <div class="card border-0 shadow mb-4">
                    ...
                    <form action="{{ route('account.updateProfile') }}" method="POST" id="userForm" name="userForm">
                        @csrf
                        <input type="hidden" name="_method" value="PUT">  <!-- Override to PUT method -->
...
$("#userForm").submit(function(e){
    ...
    var formData = $("#userForm").serialize(); // Will include _method=PUT automatically

    $.ajax({
        url: "{{ route('account.updateProfile') }}",
        type: "POST", // Keep it POST, but _method=PUT will simulate PUT
        dataType: 'json',
        data: formData,
        success: function (response) {
            if(response.status == true){
                $("#name, #email, #designation, #mobile").removeClass('is-invalid')
                    .siblings('p')
                    .removeClass('invalid-feedback')
                    .html('');
                window.location.href = "{{ route('account.profile') }}";  // Redirect after success
            } else {
